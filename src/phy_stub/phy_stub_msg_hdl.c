...
S16 l1HdlUlTtiReq(uint16_t msgLen, void *msg)
{
   ...
   while(numPdus)
   {
      if(ulTtiReq->pdus[numPdus-1].pduType == 0)
      {
         ...
         // /* Send RACH Ind to L2 for fifth UE */
         // if(phyDb.ueDb.ueCb[UE_IDX_5].rachIndSent == false && phyDb.ueDb.ueCb[UE_IDX_4].msgRrcReconfigComp == true)
         // {
         //    phyDb.ueDb.ueCb[UE_IDX_5].isCFRA = false;
         //    phyDb.ueDb.ueCb[UE_IDX_5].rachIndSent = true;
         //    l1BuildAndSendRachInd(ulTtiReq->slot, ulTtiReq->sfn, CB_RA_PREAMBLE_IDX);
         //    phyDb.ueDb.numActvUe++;
         // }

         /* Send RACH Ind to L2 for fifth UE */
         // if(phyDb.ueDb.ueCb[UE_IDX_6].rachIndSent == false && phyDb.ueDb.ueCb[UE_IDX_5].msgRrcReconfigComp == true)
         // {
         //    phyDb.ueDb.ueCb[UE_IDX_6].isCFRA = false;
         //    phyDb.ueDb.ueCb[UE_IDX_6].rachIndSent = true;
         //    l1BuildAndSendRachInd(ulTtiReq->slot, ulTtiReq->sfn, CB_RA_PREAMBLE_IDX);
         //    phyDb.ueDb.numActvUe++;
         // }
#endif       
