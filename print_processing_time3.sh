...
sec_pattern="([0-9]+\.[0-9]+) us"

# run the odu command and continuously monitor its output
while IFS= read -r line; do
    # check if the line contains "processing time"
    if [[ $line == *"JOJO processing time Measurement : Processing Time of Scheduling Algorithm:"* ]]; then
        # extract the processing time value from the line
        processing_time=$(echo "$line" | grep -oE "$sec_pattern" | sed 's/ us//')
